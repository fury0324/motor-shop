...
function customerReminderEmail(opts: {
  customerName: string; transactionNo: string; itemName: string; dueDate: string; amountDue: number;
}) {
  return `
    <html><head><style>
...
function staffAlertEmail(opts: {
  staffName: string;
  dueDate: string;
  rows: { customerName: string; transactionNo: string; itemName: string; amountDue: number; customerContact: string }[];
}) {
...
// plumbing: this writes to mail/ (Cloud Function, Admin SDK) and the
// Trigger Email extension does the actual sending, instead of the original
// hardcoding Gmail credentials in the PHP file itself.
async function runDuePaymentReminders() {
  ...
  const duePayments = dueSnap.docs.map((paymentDoc) => {
    ...
    return {
      ...
      dueDate: paymentDoc.data().dueDate as string,
      transactionNo: (transaction as Record<string, unknown>).transactionNo as string,
      customerId: (transaction as Record<string, unknown>).customerId as string,
      customerName: (transaction as Record<string, unknown>).customerName as string,
      customerContact: (transaction as Record<string, unknown>).customerContact as string,
      itemName: (transaction as Record<string, unknown>).inventoryName as string,
    };
  });
  ...
  const customerEmailById = new Map(customerDocs.map((d) => [d.id, d.data()?.email as string | undefined]));

  const mailCollection = db.collection("mail");
  let customersNotified = 0;
  const customerFailed: string[] = [];
...
// Runs daily; replaces the original's setInterval-driven client polling
// (and the on-demand PHP endpoint it called) with a real server-side job.
export const sendDuePaymentReminders = onSchedule(
  { schedule: "every day 08:00", timeZone: "Asia/Manila" },
  async () => {
    await runDuePaymentReminders();
  }
);

// Manual trigger for admins (support/testing use — e.g. resending today's
// reminders after fixing a customer's email) without waiting for the
// schedule.
export const sendDuePaymentRemindersNow = onCall(async (request) => {
