...
function engineIndexRef(db, engineNumber) {
  return db.collection('unitEngineNumbers').doc(engineNumber)
}
function chassisIndexRef(db, chassisNumber) {
...
router.post('/addInventoryUnit', callable(async (request) => {
...
router.post('/deleteInventoryUnit', callable(async (request) => {
  assertStaffOrAbove(request.auth)
...
// Mirrors delete-inventory.php's cascade plus the RESTRICT checks that
// MySQL enforced via transactions.inventory_id and parts_transactions.inventory_id.
router.post('/deleteInventoryItem', callable(async (request) => {
  assertStaffOrAbove(request.auth)
  ...
  if (!txSnap.empty || !partsTxSnap.empty) {
    throw new HttpsError('failed-precondition', 'This item has sales history and cannot be deleted.')
  }
